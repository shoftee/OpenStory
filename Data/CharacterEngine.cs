...
namespace OpenMaple.Data
{
    static class CharacterEngine
    {
        ...
        private const string SelectCharacterById = "SELECT TOP 1 * FROM [dbo].[Character] WHERE [CharacterId]=@characterId";
        ...
        public static void SaveKeyBindings(int characterId, List<KeyBinding> bindings)
        {
            ...
            {
                throw new ArgumentException("There must be exactly " + KeyLayout.KeyCount + " bindings in the given list.");
            }
            int count = 0;
            ...
            foreach (var binding in bindings.Where(b => b.HasChanged))
            {
