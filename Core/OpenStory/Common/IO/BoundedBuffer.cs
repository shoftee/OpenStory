...
namespace OpenStory.Common.IO
{
    ...
    public sealed class BoundedBuffer : IDisposable
    {
        ...
        /// <para>If there is not enough space for <c>count</c> elements, it will fill the remaining space with bytes from the start of <c>buffer</c>.</para>
        /// </remarks>
        ...
        private int AppendInternal(byte[] buffer, int offset, int requested)
        {
            ...
            this.MemoryStream.Write(buffer, offset, stored);
        ...
        public byte[] ExtractAndReset(int newCapacity)
        {
            ...
            {
                data = this.MemoryStream.GetBuffer();
            }
